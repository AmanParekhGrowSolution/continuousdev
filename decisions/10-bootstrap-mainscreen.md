# [Issue #10] Bootstrap calendar app shell with MainScreen showing today's date
...
`Locale` for the formatter: use `Locale.getDefault()` (default `DateTimeFormatter.ofPattern` behaviour) so the date matches the device locale. The acceptance-criteria example "Tuesday, 28 April 2026" is en-GB style; en-US devices will render "Tuesday, April 28, 2026" only if the pattern differs, but the literal pattern `"EEEE, d MMMM yyyy"` produces "Tuesday, 28 April 2026" regardless of locale (only the names of weekday/month are localised). This matches the acceptance criteria literally.
...
- `strings.xml` — the date string is computed at runtime, not a localised string resource. The format pattern itself is a developer-facing constant, not user-facing copy. (If a future issue wants to localise the *pattern* per locale, that's a separate decision.)
